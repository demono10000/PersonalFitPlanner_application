  ...
  Future<void> createTrainingPlan() async {
    if (_formKey.currentState!.validate()) {
      ...
      final response = await http.post(
        Uri.parse('${Config.apiUrl}/training_plans/create/'),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer $accessToken',
        },
      ...
      if (response.statusCode == 201) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Training plan created successfully')),
        );
        Navigator.pop(context, true);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to create training plan')),
        );
      }
    }
  }
